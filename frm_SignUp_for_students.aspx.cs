...
public partial class _Default : System.Web.UI.Page
{
    ...
    {
        QRY = "SELECT MAX(Stud_Id) + 1 FROM Tbl_StudentPersonalDetails";
        CNN = new SqlConnection(CNS);
        CMD = new SqlCommand(QRY, CNN);
        CNN.Open();
        ...
        if (DR.Read())
        {
            StudId = int.Parse(DR[0].ToString());
        }
        DR.Close();
        CNN.Close();
        CMD.Dispose();

        QRY = "INSERT INTO Tbl_StudentPersonalDetails VALUES (";
        QRY += ""+StudId+", ";
        QRY += "'"+txtname.Text+"', ";
        ...
        Response.Redirect("frm_Educational_Details.aspx");
